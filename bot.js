...
async function WhatsAlexa () {
    ...
    conn.browserDescription = ["WhatsAlexa-Bot", "Safari", '1.0.0']
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅ Whats Alexa Working!')
       );
         ...
         if (config.LANG == 'EN') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/WhatsAlexa.png"), MessageType.image, { caption: `『 WhatsAlexa 』\n\nHello ${conn.user.name}!\n\n*🆘 General Help For You! 🆘*\n\n🔹 *#alive:* Check if the bot is running.\n\n🔹 *#list:* Shows the complete list of commands.\n\n🔹 *#restart:* It Restarts the bot.\n\n🔹 *#shutdown:* It Shutdown/Turn off the bot.\n\n *⚠ Warning, If you shutdown/turn off, there is no command to turn on the bot So You must got to heroku & turn on the worker. ⚠*.\n\nThank You For Using WhatsAlexa 💖`});
              await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/bgm/bott.mp3"), MessageType.audio, { mimetype: Mimetype.mp4Audio, ptt: true});
     ...
     conn.on('chat-update', async m => {
        ...
        if (config.WELCOME == 'pp' || config.WELCOME == 'Pp' || config.WELCOME == 'PP' || config.WELCOME == 'pP' ) {
            ...
            if (gb !== false) {
               ...
               await conn.sendMessage(msg.key.remoteJid, fs.readFileSync("./src/bgm/gby.mp3"), MessageType.audio, { mimetype: Mimetype.mp4Audio, ptt: true});
            ...
            if (gb !== false) {
                ...
                await axios.get(pp, {responseType: 'arraybuffer'}).then(async (res) => {
                    //created by afnanplk
                await conn.sendMessage(msg.key.remoteJid, res.data, MessageType.image, {thumbnail: White.tm_b, caption:  gb.message.replace('{pp}', '').replace('{gphead}', pinkjson.subject).replace('{gpmaker}', pinkjson.owner).replace('{gpdesc}', pinkjson.desc).replace('{owner}', conn.user.name).replace('{time}', time).replace('{mention}', tag), contextInfo: {mentionedJid: [msg.messageStubParameters[0]]} }); });                           
           await conn.sendMessage(msg.key.remoteJid, fs.readFileSync("./src/bgm/wel.mp3"), MessageType.audio, { mimetype: Mimetype.mp4Audio, ptt: true});
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/WhatsAlexa.png"), MessageType.image, { caption: '*『 ERROR 』*\n\n*WhatsAlexa an error has occurred!*\n_Report this error to the developer! [ SHEFIN KL14 ]._\n\n*Error:* ```' + error + '```\n\n' });
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/bgm/error.mp3"), MessageType.audio, { mimetype: Mimetype.mp4Audio, ptt: true});
                            } else if (config.LANG == 'ML') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/WhatsAlexa.png"), MessageType.image, { caption: '*『 ERROR 』*\n\n*WhatsAlexa error സംഭവിച്ചു!*\n_ഈ error  ഡെവലപ്പറെ അറിയിക്കുക! [ SHEFIN KL14 ]._\n\n*error:* ```' + error + '```\n\n' });
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/bgm/error.mp3"), MessageType.audio, { mimetype: Mimetype.mp4Audio, ptt: true});
                            } else {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/WhatsAlexa.png"), MessageType.image, { caption: '*『 KESALAHAN 』*\n\n*WhatsAlexa telah terjadi kesalahan!*\n_Laporkan kesalahan ini ke pengembang [ SHEFIN KL14 ]._\n\n*Kesalahan:* ```' + error + '```\n\n' });
                            }
                        }
                    }
                }
